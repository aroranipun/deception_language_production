...
final_features_raw = final_responses %>% 
  select(session_id,response_box, response,truth,tag,prompt_id,response_start,response_finish,version) %>%  
  left_join(
    y = prompts,
    by = "prompt_id"
    ) %>%
...
# Write the table------------------------------------------------
final_features = write.csv(x = final_features,file = "data/engineered/final_features.csv",row.names = F)
